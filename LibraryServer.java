...
public class LibraryServer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        server.createContext("/api/checkout", ex -> {
            if(ex.getRequestMethod().equals("OPTIONS")){ send(ex,200,""); return; }
            String b=body(ex);
            String uid=getJson(b,"userId"), bid=getJson(b,"bookId"), requesterId=getJson(b,"requesterId");
            User requester=findUser(requesterId);
            if(requester==null||(!requester.id.equals(uid)&&!requester.role.equals("admin"))){ send(ex,403,"{\"error\":\"Not authorized\"}"); return; }
